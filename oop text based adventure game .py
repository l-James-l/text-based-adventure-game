...
class Enemie:
    def __init__(self, name, health, damage, hit_chance, dialog):
        self.name = name
        self.health = health
        self.damage = damage
...
class Prompts(Cmd):
    ...
    def do_attack(self, args):
        ...
        if len(command_split) != 3:
            ...
            weapon = command_split[2]
            enemie = command_split[0]
            valid_enemie = False
...
officer = Enemie("officer", 15, 5, 70, "'hey you, get back in your cell!!' \n*brandshes batton*")
...
def make_items():
    global nail, paper, batton, cake, apple, knife, RPG, code_paper, easter_egg
    
    nail = Item("nail", "a", [("attack", 20, 20), "unlock"])
    paper = Item("paper", "a peice of", [("attack", 2, 90), ("read", "Zork intro text.txt")])
    code_paper = Item("code paper", "the", [["attack", 0, 100], ["read", "Zork code.txt"]])
...
def look(current_room):
    ...
    if len(current_room.enemies) == 0:
        ...
        print("this room has a", (current_room.enemies[0]).name, "in it!")
    ...
    for i in range(0, len(current_room.rooms)):     
        ...
        if current_room.rooms[i][0] == direction:
            if type(current_room.rooms[i][1]) == type(cell):
                ...
                if rooms_split[0] == "locked" or rooms_split[0] == "closed" or rooms_split[0] == "shut":
